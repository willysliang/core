...
- [Vite](https://vitejs.dev/) — 一种新型前端构建工具，能够显著提升前端开发体验。
- [Webpack](https://webpack.js.org/) — 现代 JavaScript 应用程序的静态模块打包器。主要目的是捆绑 JS 文件可以在浏览器中使用，但它也能够转换、捆绑或打包几乎任何资源。
- [Parcel](https://parceljs.org/) — 一个 Web 应用程序捆绑器，以其开发人员体验而著称。它利用多核处理提供了极快的性能，并且需要零配置。
- [Rollup](https://rollupjs.org/) — 用于 JS 的模块打包器，它将小段代码编译成更大更复杂的东西，例如库或应用程序。
...
```bash
- Webpack 是一种用于编译 JavaScript 模块的工具，又称模块捆绑器（js应用的静态模块化打包工具）。具有打包压缩、编译兼容、能力扩展等功能。其最初的目标是实现前端项目的模块化，也就是如何更高效地管理和维护项目中的每一个资源。
...
```bash
使用 webpack 可以在 js 代码中使用 import 或 require 语法。
Webpack 旨在处理所有的依赖关系，而不仅只是 js，而 loader 是实现此目的的一种方式。
...
三、综上所述，开发模式下 vite 比 wekpack 快的原因
  ...
  2. vite 在解析模块依赖关系时，利用了 esbuild，更快（esbuild 使用 Go 编写，并且比以 js 编写的打包器预构建依赖快 10-100 倍）；
