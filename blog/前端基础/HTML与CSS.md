...
```bash
WEB 标准可分为结构、表现和行为。
	- 结构层(structural layer)：主要由 HTML 或 XHTML 之类的标记语言负责创建。在页面 body 中写入的标签都是为了页面结构，对网页内容的语义含义做出描述。
	- 表示层(presentation layer)： 由 CSS 样式表负责创建，通过 CSS 可以使页面结构标签更具美感。
	- 行为层(behaviorlayer)：主要由 js 组成，指页面和用户具有一定的交互，同时页面结构或者表现发生变化。

W3C 的代码规范
	- 书写标签必须闭合、标签小写、不乱嵌套，标签规范可以提高搜索引擎对页面的抓取效率，有利于 SEO。
...
2. 自定义标签：
    - XML 没有默认的标签，所有的标签都是自己定义（XML只有双标签）
    - XML 中必须要有一个根节点，所有的子节点都放置在根节点下
3. XML 解析：
		- 因为 XML 就是标签，所以可以直接用解析 Dom 元素的方法解析 XML


### XML 与 html 的区别
1. xml 是可扩展标记语言，类似 html，但xml标签没有被预定义，需要自行定义标签。
2. xml 用来传输和存储数据，焦点是数据内容；html 被设计用来显示数据，焦点是数据外观；
3. xml 具有自我描述性。
```
...
- XML 是可扩展标记语言，XML 和 HTML 最大区别在于 XML 的标签可以自己创建，数量无限多，而 HTML 的标签名都是固定且数量有限。
- XHTML 是一个基于 XML 的标记语言，他与 HTML 没有本质区别，但比 HTML 严格。



### HTML 与 XHTML 的区别
为了规范 HTML，W3C 结合 XML 制定了 XHTML1.0 标准，这个标准按照 XML 的要求来规范 HTML：
    - 文档顶部 `doctype` 声明不同，XHTML 的 doctype 顶部声明中明确规定了 xhtml DTD 的写法
    - 元素必须始终正确嵌套（所有的XML标记都必须合理嵌套）
    - 标签必须始终关闭（必须有一个相应的结束标记）
    - 标签名必须小写
    - 特殊字符必须转义（所有 `<` 和 `&` 特殊符号用编码表示）
    - 文档必须有根元素
    - 属性值必须用双引号 `""` 括起来
    - 禁止属性最小化（属性必须要赋值：例如，必须使用 `checked="checked"` 而不是 `checked`）
		- 不要在注释内容中使 `--`
		- 图片必须有说明文字
```
