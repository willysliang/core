...
### Promise 的状态
1. Promise 对象代表异步操作，依照 `Promise/A+` 的定义，`Promise` 有四种状态，且不受外界影响:
		- pending: 等待状态，在等响应跳转。
		- resolved/fulfilled: 满足状态，当我们主动回调resolve时处于该状态，并且会回调`.then()`。
		- rejected: 拒绝状态，当我们主动回调了reject时处于该状态，并且会回调`.catch()`。
		- settled: Promise已被 fulfilled 或 rejected，且不是 pending（`fulfilled` 与 `rejected` 一起合称 `settled`）
...
- 优点：
		...
		- Promise 对象提供统一的接口，可以将异步操作以同步操作的流程表达出来，避免层层嵌套的回调函数(解决了回调地狱问题)
...
Promise.then 是微任务，setTimeout 触发的函数回调是宏任务的异步代码，同一个事件循环中，Promise.then 总是先于 setTimeout 回调函数的执行。
```
