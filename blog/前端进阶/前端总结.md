...
ModifiedTime: 2025-12-03 11:08:02
Description: 前端总结
---
...
## . http 和 https 的区别
- http 传输的数据都是明文(未加密)，https 协议是由 http 和 ssl 协议构建的可进行加密传输和身份认证的网络协议，对 http 协议传输的数据进行加密处理，比 http 协议的安全性更高。
...
```bash
当用户更新静态资源（如 JS、CSS、图片）时，由于 CDN 缓存，用户可能仍然访问到旧版本的资源。这会导致问题，尤其是当资源内容发生不兼容变更时。

1. 构建时调整资源的版本号/哈希值
			如 `script.js?v=1.0.1` 或 `script.[hash].js`
2. CDN缓存手动刷新 / 自动化刷新CDN脚本
3. 利用 Service Worker 控制缓存
4. HTML文件设置 no-cache
```
...
```bash
1. 单向数据流：数据通过props从父节点传递到子节点，如果父级的某个props改变，react会重新渲染所有的子节点
2. 高效：react通过虚拟DOM，最大限度减少与dom的交互
3. 组件化：通过react构建组件，把页面功能拆分，让代码更容易复用/维护
```
...
相同点：
    1. 都支持服务端渲染
    2. 都有虚拟dom、组件化开发、通过props参数进行父子组件数据传递
    3. 都实现webcomponent规范
    4. 都是数据驱动视图
    5. 都有状态管理，react有redux，vue有vuex
    6. 都支持native的方案，react有react native，vue有weex

不同点：
		1. react严格上只针对mvc的view层，vue是mvvm模式
		2. 虚拟dom不同。vue会跟踪每一个组件的依赖关系，不需要重新渲染整个dom组件树；react当应用的状态被改变时，全部组件都会重新渲染，所以react中用`shouldComponentUpdate`钩子函数来控制
		3. 组件写法不同，react是jsx+inline style，即把html和css都写进js；vue则是html、css、js在同一个文件
		4. 数据绑定不同，vue实现数据双向绑定，react数据流动是单向的
		5. 在react中state需要用setstate方法进行更新状态；在vue中state对象不是必须的，数据由data属性在vue对象中管理
```
...
#### 在react组件的何处发起ajax请求？
在react组件中，应在componentDidMount钩子函数中发起网络请求。这个方法会在组件第一次挂载(被添加到DOM)时执行，在组件的生命周期中仅会执行一次。更重要的是，你不能保证在组件挂载之前ajax请求已经完成，如果是这样尝试在一个未挂载的组件上调用setState，这将不起作用。在componentDidMount中发起网络请求将保证这一个组件可以更新
```
...
```bash
1. 抹平了浏览器之间的兼容问题
2. 对所有事件中心化管控，在组件挂载销毁时统一订阅和移除事件（事件委托）
		事件池、统一监听、数据变更时批量订阅和销毁
3. 减少内存：引入事件池避免垃圾回收，再事件池获取或释放事件对象，避免频繁的创建和销毁
```



### 组件的状态state和属性props的区别
...
发展规划
入职一家新公司后的话：
1. 短期目标(1-2年)：深度融入团队与业务，掌握公司技术栈、开发流程与业务，成为项目中值得信赖的开发力量。
2. 中期目标(3-5年)：希望能够参与甚至主导前端架构的设计和技术选型，比如在性能优化、工程化体系建设、微前端落地等方面进行深入探索和实践。
3. 长期目标(5年+)：引领与创新，希望能成长为一名在某一领域有深厚积淀的技术专家（或架构师），能结合业务发展，在前端基础建设、用户体验优化、或者通过新技术（如AI+前端）驱动业务创新等方面，为团队和公司创造更大的价值。
