...
```bash
二叉搜索树 BST(Binary Search Tree)专注于检索，特点是让节点按照一定的规律摆放，从而让搜索某个节点特别的高效。
		- 每一个节点上最多两个子节点；
		- 任意节点左子树的节点值都小于当前节点；
		- 任意节点右子树的节点值都大于当前节点。
...
```bash
形如二叉搜索树可能存在某一边的子节点过多，造成树的深度过大。
平衡二叉树：规定任意节点左右子树高度差不超过 1。
		- 高度平衡；
...
![image-20260101164816712](./image/image-20260101164816712.png)

![image-20260101164949892](./image/image-20260101164949892.png)

![image-20260101165122688](./image/image-20260101165122688.png)

![image-20260101165253365](./image/image-20260101165253365.png)
...
红黑规则：
	1. 每一个节点是红色，或者黑色；
	2. 根节点必须是黑色；
	3. 如果一个节点没有子节点或父节点，则该节点相应的指针属性值为 Nil，每个叶节点(Nil)是黑色的；
	4. 如果某一个节点是红色，它的子节点必须是黑色（两个红色节点不能相连）；
	5. 每一个节点，从该节点到其所有后代叶节点的简单路径上，黑色节点数目相同；
...
![image-20260101170347032](./image/image-20260101170347032.png)

![image-20260101172915949](./image/image-20260101172915949.png)
