...
> 2. 状态模式：根据状态来分离和选择行为
> 3. 命令(事务)模式：不同对象间约定好相应的接口（项目负责人分配任务给相应的工作人员，而无需知道任务怎么完成。	封装请求）
> 4. 备忘录模式：保存和恢复内部状态
...
> 10. 观察者(发布订阅)模式：当观察对象发生变化时，自动调用相关函数（订阅消息，发布通知消息更新。	vue双向绑定）
...
>
>```js
>const books = new Array(10000).map((_, index) => {
...
>      // 将家长和孩子放到一起存入花名册
>      ;(
>        this.matchBook[person.family] || (this.matchBook[person.family] = [])
>      ).push(person)
>    })
>  },
>
...
>          // 遍历该家庭中注册到名册上的所有成员
>          .forEach((enemyFamily) =>
>            this.matchBook[enemyFamily].forEach((enemy) =>
>              this.matchBook[familyName]
...
>    result
>      ? console.log(
>          `${they.name}: 我觉得合适~ \t (我的要求 ${self.name} 已经满足~)`,
>        )
>      : console.log(
>          `${they.name}: 你是个好人! \t (我的要求 ${self.name} 不能满足！)`,
>        )
>  },
>}
>
